#!/usr/bin/env python3
"""
VLESS Key Checker — v2.1
Источник: открытые публичные репозитории GitHub
Цель: учебное исследование доступности публичных VLESS-конфигов

Улучшения v2.1:
  - Исправлена опечатка в URL kort0881 (vless.tx → vless.txt)
  - Источники проверяются параллельно (быстрее загрузка)
  - Недоступные источники помечаются предупреждением, а не роняют всё
  - GeoIP: повтор при rate-limit вместо немедленного N/A
  - Прогресс-бар теперь показывает оставшееся время (ETA)
  - Дедупликация ключей до проверки учитывает только хост:порт+uuid
  - Финальная статистика по источникам (сколько ключей дал каждый)
"""
